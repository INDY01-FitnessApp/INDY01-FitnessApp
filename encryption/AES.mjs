...
function encrypt(text, key) {
  // Generate round keys
  const roundKeys = expandKey(key).slice(0, 3);
  const numRounds = 2;
  // Break text into blocks
  const chunks = chunkTextIntoBytes(text, 16);
  ...
  chunks.forEach((chunk) => {
    ...
    for (let i = 1; i <= numRounds; i++) {
      let pass = encryptRound(lastPass, roundKeys[i], i == numRounds);
...
// Text should be a string of bytes in hex
function decrypt(text, key) {
  const roundKeys = expandKey(key).slice(0, 3).reverse();
  const numRounds = 2;

  // Chunk hex text into 32 hex characters (128 bytes)
  let numChunks = Math.ceil(text.length / 32);
  let chunks = [];
  for (let i = 0; i < numChunks; i++) {
    chunks.push(text.substring(i * 32, (i + 1) * 32));
  }
  ...
  chunks.forEach((chunk) => {
    ...
    for (let i = 0; i < numRounds; i++) {
      let pass = decryptRound(lastPass, roundKeys[i], i == 0);

      lastPass = pass;
    }
    // XOR with last round key
    lastPass = XORHexStrings(lastPass, roundKeys[numRounds]);
    decryptedText += lastPass;
  });

  // Convert decrypted hex string to array of character codes, then translate to the original text
  let translatedText = "";
  for (let i = 0; i < decryptedText.length; i += 2) {
    let byte = decryptedText.substring(i, i + 2);
    if (byte == "00") continue; // 0x00 was used for padding to keep text chunks equal size, does not need to be included in translation
    let charCode = hexStringToNumber(byte);
    translatedText += String.fromCharCode(charCode);
  }
...
function encryptRound(text, key, isLastRound) {
  let result = text;
  // // Substitute bytes
...
function decryptRound(text, key, isFirstRound) {
  let result = XORHexStrings(text, key);
  let table = hexStringToTable(result);

  if (!isFirstRound) invMixColumns(table);
...
function invShiftRows(state) {
  ...
  for (let i = 1; i <= 3; i++) {
    let row = state[i];
    let rowEnd = row.slice(4 - i, 4);
    let rowStart = row.slice(0, 4 - i);
    state[i] = rowEnd.concat(rowStart);
  }
}
...
function invSubBytes(hexString) {
  let substitutedWord = "";
  for (let i = 0; i < hexString.length; i += 2) {
    let _byte = hexString.substring(i, i + 2);
    let _mostSignificantNibble = _byte[0];
    ...
    let _rowIndex = parseInt(_mostSignificantNibble, 16);
    let _colIndex = parseInt(_leastSignificantNibble, 16);
    let _substitutedByte = invSubTable[_rowIndex][_colIndex];
    substitutedWord += numberToHexString(_substitutedByte);
  }
  return substitutedWord;
}
...
function chunkTextIntoBytes(string, maxBytes) {
  let byteCounter = 0;
  let buildString = "";
  const chunks = [];
